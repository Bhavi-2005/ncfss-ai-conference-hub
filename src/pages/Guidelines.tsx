    ...
    <div className="min-h-screen pt-20 pb-16 px-4">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="mb-12 text-center">
          <p className="text-xs text-primary tracking-widest uppercase mb-2">Er. Perumal Manimekalai College of Engineering</p>
        ...
        {/* Abstract Guidelines */}
        <div className="gradient-card rounded-2xl p-8 glow-border mb-8">
          <div className="flex items-center gap-3 mb-6">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10">
              <FileText className="h-5 w-5 text-primary" />
            </div>
            <h2 className="font-display text-lg font-bold text-foreground">Submission Guidelines</h2>
          </div>
          <div className="space-y-3 text-sm text-muted-foreground leading-relaxed">
            ...
            <p>
              Full paper shall be in about <span className="text-foreground font-semibold">5 pages</span>.
            </p>
            <p>
              Abstract, Full paper, and presentation shall be submitted to:{" "}
              <a href="mailto:cseconference2025@gmail.com" className="text-primary hover:underline">
                cseconference2025@gmail.com
              </a>
            </p>
          </div>
        </div>
              ...
              <tbody>
                {importantDates.map((row, i) => (
                  <tr key={row.event} className={i % 2 === 1 ? "bg-secondary/50" : ""}>
                    <td className="px-6 py-3 text-muted-foreground">{row.event}</td>
                    <td className="px-6 py-3 text-primary font-medium">{row.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-secondary">
                  ...
                  <th className="px-6 py-3 text-left font-semibold text-foreground">Fee</th>
                </tr>
              </thead>
              <tbody>
                {fees.map((row, i) => (
                  <tr key={row.category} className={i % 2 === 1 ? "bg-secondary/50" : ""}>
                    ...
                    <td className="px-6 py-3 text-primary font-medium">{row.fee}</td>
                  </tr>
                ))}
              </tbody>
            </table>
